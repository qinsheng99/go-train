...
type boyDao struct {
	db *db.BundlePostgresql
}

func NewPostgresBoy(db *db.BundlePostgresql) boy.Boyimpl {
	return &boyDao{db: db}
}

func (b *boyDao) Getlist() (data []*model.Boy, err error) {
	err = b.db.Db.Model(&model.Boy{}).Find(&data).Error
	return
}
...
func (b *boyDao) FindJson(q string, flag bool) (data []*model.Boy, err error) {
	query := b.db.Db.Model(&model.Boy{}).Debug()
	if flag {
